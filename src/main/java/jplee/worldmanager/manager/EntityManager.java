...
public class EntityManager {
	...
	public void loadFromConfig(WorldManagerConfig config) {
		if(!startingItems.isEmpty()) startingItems.clear();
		if(!hostileSpawnHeightWorlds.isEmpty()) hostileSpawnHeightWorlds.clear();

		maxHostelSpawnHeight = config.getMaxHostileSpawnHeight();

		hostileSpawnHeightBlacklist = config.isHostileSpawnHeightBlackList();
		if(maxHostelSpawnHeight != -1) for(int i : config.getHostileSpawnHeightWorlds())
		...
		if(startingInventory) {
			...
			for(String item : startInv) {
				if(!item.startsWith("#") && !item.trim().isEmpty()) {
					...
					if(count >= 18) {
						WorldManager.logger.warning("There was more that 18 items listed for starting inventory",
							new Object[0]);
						break;
					}
				}
	...
	private boolean worldHasMaxHostileSpawnHeight(int world) {
		...
		return maxHostelSpawnHeight != -1
			&& (contains && !hostileSpawnHeightBlacklist || !contains && hostileSpawnHeightBlacklist);
	}
