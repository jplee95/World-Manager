...
@Deprecated
public class CommandWorldManager2 extends CommandBase {
	...
	public List<String> getCommandAliases() {
		return Arrays.asList(new String[] { "wm" });
	}
		...
		if(args.length > 0) {
			if(args[0].equals("processchunk") && args.length == 3) {
				...
				if(!world.isBlockLoaded(blockPos)) {
					...
					GenerationManager.instance.processChunk(world, new ChunkPos(chunkX, chunkZ));
